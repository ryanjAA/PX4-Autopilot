...
def load_dialect():
    ...
    mavlink_directory = root / "src/modules/mavlink"
    xml = mavlink_directory / "mavlink_m/message_definitions/v1.0/mavlink_m.xml"
    generator = mavlink_directory / "mavlink_m_generator/tools/mavgen.py"
    pythonpath = mavlink_directory / "mavlink_m_pythonpath"
    generator_python = shutil.which("python3.9")
    if generator_python is None:
        raise RuntimeError("MAVLink-M generation requires Python 3.9")
