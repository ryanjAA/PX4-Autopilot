...
VEHICLE_COMMAND = (ROOT / "msg/versioned/VehicleCommand.msg").read_text()
...
DUAL_RUNNER = (ROOT / "Tools/aags_mavlink_m/run_dual_gz.sh").read_text()
...
class InterceptPolicyTest(unittest.TestCase):
    ...
    def test_intercept_uses_internal_exact_fly_through_only(self) -> None:
        self.assertIn(
            "VEHICLE_CMD_PX4_MAVLINK_M_FLY_THROUGH = 100002", VEHICLE_COMMAND
        )
        self.assertIn(
    ...
    def test_sitl_data_tree_precedes_px4_options(self) -> None:
        self.assertRegex(
            SITL_RUNNER,
            r"str\(self\.binary\),\s*str\(self\.etc\),\s*\"-i\"",
        )
        self.assertRegex(
            DUAL_RUNNER,
