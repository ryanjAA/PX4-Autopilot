...
class TestSourcePolicy(unittest.TestCase):
    ...
    def test_protected_handler_precedes_all_generic_paths(self) -> None:
        ...
        generic = RECEIVER.index("handle_message(&msg);", protected)
        gimbal = RECEIVER.index("handle_messages_in_gimbal_mode(msg);", protected)
        self.assertLess(route_gate, protected)
        self.assertLess(protected, generic)
        self.assertLess(protected, gimbal)
    ...
    def test_mavlink_m_cmake_stays_compatible_with_declared_cmake_310(self) -> None:
        ...
        for contract in (
            "RESULT_VARIABLE MAVLINK_M_GENERATOR_GIT_RESULT",
            "ERROR_VARIABLE MAVLINK_M_GENERATOR_GIT_ERROR",
            'if(NOT "${MAVLINK_M_GENERATOR_GIT_RESULT}" STREQUAL "0")',
            "RESULT_VARIABLE MAVLINK_M_PYTHON_VERSION_RESULT",
            "ERROR_VARIABLE MAVLINK_M_PYTHON_VERSION_ERROR",
            'if(NOT "${MAVLINK_M_PYTHON_VERSION_RESULT}" STREQUAL "0")',
        ):
